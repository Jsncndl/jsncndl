import styled from 'styled-components'
import { useScrollTo } from '../../utils/hooks/useScrollTo'
import backgroundWave from '../../images/svg/wave.svg'
import { colors } from '../../styles/colors'

const StyledLinksContainer = styled.div`
  display: flex;
  flex-direction: row;
  flex-wrap: wrap;
  justify-content: space-around;
  align-items: center;
  padding: 15px;
`

const SitesLink = styled.div`
  display: flex;
  flex-direction: column;
  justify-content: center;
  align-items: center;
  width: 90px;
  height: 90px;
  text-align: center;
  padding: 10px;
  margin: 10px;
  background: linear-gradient(180deg, rgba(53,57,65,1) 11%, rgba(38,40,43,1) 37%);
  box-shadow: ${colors.third} 0 0 5px 2px;
  border-radius: 20px;
  transition: all 500ms ease-in-out;
  &:hover {
    transform: scale(1.1) rotate(5deg);
  }
`
const SitesLogo = styled.img`
  width: 50px;
  height: 50px;
`
const StyledContainer = styled.div`
  padding: 15px;
`
const StyledTitle = styled.h1`
  padding: 15px;
  text-align: center;
  margin: 0;
`
interface toggleClickProps {
  path: 'home' | 'sites' | 'contact' | 'more'
}

export const Home = () => {
  const toggleClick = ({ path }: toggleClickProps) => {
    document.getElementById(path)?.scrollIntoView({ block: 'start' })
  }

  return (
    <span
      style={{
        position: 'absolute',
        height: '97vh',
        width: '100%',
        backgroundColor: `${colors.first}`,
        backgroundImage: `url(${backgroundWave})`,
        backgroundRepeat: 'no-repeat',
        backgroundPositionX: '100%',
        backgroundPositionY: '100%',
        backgroundSize: 'contain',
      }}>
      <StyledTitle>Josian Candel - Développeur Web</StyledTitle>
      <StyledContainer>
        <p>
          <strong>Développeur web</strong> porté plutôt sur le <strong>frontend</strong>, je
          recherche un emploi à plein temps ou une alternance pour effectuer la formation
          Développeur d'Application JavaScript React avec l'organisme OpenClassrooms.
        </p>
        <p>
          Je code essentiellement en <strong>React</strong>, j'ai utiliser plusieurs langages et
          technos backend pour compléter mes projets.
        </p>
      </StyledContainer>
      <StyledLinksContainer>
        <SitesLink id='more-button' onClick={() => toggleClick({ path: 'more' })}>
          <SitesLogo
            src='data:image/png;base64,'
            alt='Logo de la partie en savoir plus'
          />
          <p>Mon parcours</p>
        </SitesLink>
        <SitesLink id='sites-button' onClick={() => toggleClick({ path: 'sites' })}>
          <SitesLogo
            src='data:image/png;base64,'
            alt='Logo de la partie réalisation'
          />
          <p>Mes réalisations</p>
        </SitesLink>
        <SitesLink id='contact-button' onClick={() => toggleClick({ path: 'contact' })}>
          <img
            src='data:image/png;base64,'
            alt='Logo de la partie contact'
          />
          <p>Me contacter</p>
        </SitesLink>
      </StyledLinksContainer>
    </span>
  )
}
